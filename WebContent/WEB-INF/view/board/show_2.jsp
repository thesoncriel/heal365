...
<s:if test="showData.depth < 1"><a href="write?id=${param.id}&board_no=${param.board_no}&depth=${showData.depth+1}&re=re&subject=${showData.subject}&amp;contents=${param.contents}" class="artn-button board">답글</a></s:if>
				...
				<div class="commentItem">
					...
					<span class="commentWrap">
						...
						<s:if test="(auth.isAdmin) || (id_user == #session.user.id)">				
							<a class="artn-button board before comment_edit" href="#">수정</a>
							<a class="artn-button board before comment_delete" href="commentDelete?comment_id=${id}&id=${param.id}&board_no=${param.board_no}&amp;contents=${param.contents}">삭제</a>
...
<input type="submit" class="artn-button board" value="보내기"/>
</s:if>
<s:else>
