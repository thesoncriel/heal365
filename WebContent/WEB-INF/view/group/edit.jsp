...
<a:file id="file_banner" name="file_banner" value="${showData.file_banner }" path="/upload/group/banner/"/></td></tr>
...
<td><input type="text" id="autocomplete_user_name" data-url="/user/list?json=true" data-format="{name} ({date_birth}) :: {id}" data-field="#name,date_birth,@id:id_user" data-type="map" data-minlen="1" name="user_name" value="${showData.user_name }" required="required"/>
자동완성 기능입니다
...
<td><a:phone id="phonebox_phone_group" name="phone_group" value="${showData.phone_group }" required="required"/></td></tr>
...
<td><input type="text" id="textbox_zipcode_group" class="zipcode" name="zipcode_group" maxlength="7" data-rule="zipcode" data-to="#textbox_address_group1" data-tonew="#textbox_address_group_new" value="${showData.zipcode_group }" title="그룹이 위치하는 곳의 우편번호를 선택하세요. (클릭)" required="required"/></td></tr>
