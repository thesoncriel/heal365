    ...
    <div class="section">
		<div class="article member-join step2">
			...
			<form id="joinForm" action="join" method="post" enctype="multipart/form-data" class="validator">
				...
				<td><input type="text" id="datepicker_date_birth" name="date_birth" required="required" value="${showData.date_birth }" data-year="1900" title="생년월일을 입력 하세요. (클릭)" /></td></tr></s:if>
				...
				<td><input type="text" id="textbox_zipcode_home" name="zipcode_home" maxlength="7" data-rule="zipcode" data-to="#textbox_address_home1" data-tonew="#textbox_address_home_new" value="${showData.zipcode_home }" title="거주하는 곳의 우편번호를 선택하세요. (클릭)"/></td></tr></s:if>
				
				<s:if test="userEdit(13)"><tr>
				<th><label for="textbox_address_home2">주소</label></th>
				<td>지번: <input type="text" id="textbox_address_home1" name="address_home1" value="${showData.address_home1 }" maxlength="100" title="우편번호 선택 시 자동으로 입력 됩니다. (클릭)"/><br/>
					...
					상세: <input type="text" id="textbox_address_home2" name="address_home2" value="${showData.address_home2 }" maxlength="100" title="상세 주소를 입력 하세요."/></td></tr></s:if>
