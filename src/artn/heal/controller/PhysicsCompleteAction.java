...
public class PhysicsCompleteAction extends AbsHeal365ActionController {
	...
	public String list() throws Exception {
		...
		dbm().open();
		doShowSub("fieldInfo", "physics-complete-conf-all");//필드정보
		doList("physics-complete-all");//데이터
		dbm().close();
		return SUCCESS;
	}
	...
	public String show() throws Exception {
		...
		//검토자 Select 2013.12.30 BrunoSong 수정 요망
		...
		if (getArrayParams().containsKey("field_value") == true){
			...
			for(Integer index : liIndex){
				...
				if (getParams().get("field_type").equals("0")){
					getParams().put("field_result", dbm().selectOneString("physics-field-getResult", getParams()) );
				}
				else{
	...
	protected void mergeParamsWithEmptyDataToNull(int index, String... keys){
		...
		System.out.println(getArrayParams());
	...
	public String stats() throws Exception {
		...
		dbm().open();
		doShowSub("fieldInfo", "physics-complete-conf-all", "GroupCnt", "physics-complete-stats-group-count");//필드정보
		String sField = "";
		...
		doList("physics-complete-stats-user-count");
		dbm().close();
