...
public class PhysicsProgressAction extends AbsHeal365ActionController {
	...
	public String list() throws Exception {
		doList("physics-progress-all"); //TODO: 그룹정보 & 항목정보 조인하여 표현
		return SUCCESS;
	}
	...
	public String show() throws Exception {
		try{
			...
			dbm().open();
			lmResult = dbm().selectNonOpen("physics-complete-sing", getParams());
			if(lmResult.size() == 0){
				dbm().close();
			...
			getParams().put("id_task", getParams().get("id_complete"));
			doList("physics-complete-single");
			doShowSub("approval","approval-process-all");
			getListData().get(0).putAll(getSubData().get("approval").get(0));
	...
	public String write() throws Exception {
		...
		int iIdGroup = 0;		
		try{
			iIdGroup = Integer.parseInt(getMedicalInfo().getIdMedi());			
		} catch(Exception e){}		
		
		readApprover(iIdGroup);
	...
	public String modify() throws Exception {
		...
		if (getArrayParams().containsKey("field_value") == true){
			...
			lmResult = dbm().selectNonOpen("physics-complete-conf-all", getParams());
			...
			dbm().updateNonCommit("physics-complete-modify", getParams());
			getParams().put("id_complete", Integer.parseInt(dbm().selectOne("common-inserted-id", getParams()).get("id").toString()));

			getParams().put("id_task",getParams().get("id_complete"));
			valid.checkEmptyValue(getParams(), "", "id_previewer","approver_name","comment_previewer");
			getParams().put("date_preview", util.getNow());
			getParams().put("date_approve", util.getNow());
			getParams().put("task_type","physics");
			
			doEdit("approval-process-modify");
			...
			for(Integer index : liIndex){
				...
				mergeParamsWithEmptyDataToNull(
					index, "id");
				dbm().updateNonCommit("physics-data-modify", getParams());
			}
	...
	public String complete() throws Exception{
		...
		dbm().open();
		lmCompleteTarget = dbm().selectNonOpen("physics-data-complete-target", getParams());		
		iMax = dbm().selectOneInteger("physics-data-getMaxIdComplete", getParams());
		getParams().put("id_complete", iMax);

		for(Map<String, Object> map : lmCompleteTarget){
			getParams().put("id", map.get("id"));
			dbm().updateNonCommit("physics-data-complete", getParams());
		}
	...
	//혈압 비교 값
				...
				try{
					...
					if(!HulDang.equals("")){	
						if( (DangHwaHulDang_int  <= 12 && DangHwaHulDang_int >= 9) || (DangHwaHulDang_int  >= 12) ){
							System.out.println("헙병증의 위험 높음");
							result = "헙병증의 위험 높음";
							return result;
						}else if(DangHwaHulDang_int  >= 7 && DangHwaHulDang_int  <= 8){
							System.out.println("합병증의 위험 있음");
							result = "합병증의 위험 있음";
							return result;
		...
		public String TC_check(String tc){
			...
			if(tc_int <= 200){
				...
				System.out.println("경계수치");
				result = "경계수치";
				return result;
				
			}else if(tc_int >= 241){
				System.out.println("고지혈증");
				result = "고지혈증";
				return result;
