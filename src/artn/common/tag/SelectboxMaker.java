...
public class SelectboxMaker extends ValueboxMaker {
	...
	public SelectboxMaker createTextOptions(StringBuilder sb){
		int iLen = (list != null)? list.length : 0;
		int iValue = 0;
		int iDownIndex = 0;
		...
		if (this.zero != null){
			...
			sb.append('>').append(zero).append("</option>");
			iDownIndex = 1;
			iLen++;
		}
		
		for(int i = iDownIndex; i < iLen; i++){
			sb.append("<option value=\"").append(i).append('\"');
			
			if (iValue == i){
				sb.append(" selected=\"selected\"");
			}
			sb.append('>').append(list[i - iDownIndex]).append(unit).append("</option>");
		}
	...
	public SelectboxMaker setList(String list){
		this.list = (list != null)? list.split(",") : null;
		return this;
	}
